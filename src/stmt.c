 ...
 break;
	...
	case GOTO:     walk(NULL, 0, 0);
		       ...
		       if (t == ID) {
		       	...
		       	if (p == NULL) {
				p = add_symbol(token, &stmtlabs, 0, FUNC);
				p->scope = C_LabelScope;
...
Swtch swp; int b[]; int lb, ub; {
	...
	if (u - l + 1 <= 3)
		...
		Symbol table = genident(STATIC,
			array(voidptype, u - l + 1, 0), C_LabelScope);
		(*g_code_generator->defsymbol)(table);
		cmp(LT, swp->sym, v[l], lolab);
		cmp(GT, swp->sym, v[u], hilab);
	...
	if (isptr(p->type)) {
		...
		if (q && (q->computed || q->generated))
			warning("pointer to a %s is an illegal return value\n",
				q->scope == C_ParameterScope ? "parameter" : "local");
		else if (q)
			warning("pointer to %s `%s' is an illegal return value\n",
				q->scope == C_ParameterScope ? "parameter" : "local", q->name);
