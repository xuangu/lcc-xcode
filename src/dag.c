...
int op; Node l, r; Symbol sym; {
	...
	i = (opindex(op)^((unsigned long)sym>>2))&(D_ArrayLength(buckets)-1);
...
static void kill(p) Symbol p; {
	...
	for (i = 0; i < D_ArrayLength(buckets); i++)
		for (q = &buckets[i]; *q; )
...
if (forest->op == LABELV) {
	...
	case CALL:  { Tree save = firstarg;
		      firstarg = NULL;
		      assert(tlab == 0 && flab == 0);
		      if (tp->op == CALL+B && !g_code_generator->wants_callb) {
		      	Tree arg0 = tree(ARG+P, tp->kids[1]->type,
				tp->kids[1], NULL);
			if (g_code_generator->left_to_right)
				firstarg = arg0;
			l = listnodes(tp->kids[0], 0, 0);
			if (!g_code_generator->left_to_right || firstarg) {
				firstarg = NULL;
				listnodes(arg0, 0, 0);
			}
	...
	case MOD:   { assert(tlab == 0 && flab == 0);
		      ...
		      if (g_code_generator->mulops_calls && isint(tp->type)) {
		      	list(p);
		      	cfunc->u.f.ncalls++;
		      } } break;
...
Tree cvtconst(p) Tree p; {
	...
	if (q->u.c.loc == NULL)
		q->u.c.loc = genident(STATIC, p->type, C_GlobalScope);
	if (isarray(p->type)) {
...
void gencode(caller, callee) Symbol caller[], callee[]; {
	...
	if (glevel && g_code_generator->stabsym) {
		...
		for (i = 0; (p = callee[i]) != NULL
		         && (q = caller[i]) != NULL; i++) {
			(*g_code_generator->stabsym)(p);
			if (p->sclass != q->sclass || p->type != q->type)
				(*g_code_generator->stabsym)(q);
		}
		swtoseg(CODE);
	}
	cp = codehead.next;
	for ( ; errcnt <= 0 && cp; cp = cp->next)
		switch (cp->kind) {
		case Address:  (*g_code_generator->address)(cp->u.addr.sym, cp->u.addr.base,
			       	cp->u.addr.offset); break;
		case Blockbeg: {
			       	Symbol *p = cp->u.block.locals;
			       	(*g_code_generator->blockbeg)(&cp->u.block.x);
			       	for ( ; *p; p++)
			       		if ((*p)->ref != 0.0)
			       			(*g_code_generator->local)(*p);
			       		else if (glevel) (*g_code_generator->local)(*p);
			       }
 break;
		case Blockend: (*g_code_generator->blockend)(&cp->u.begin->u.block.x); break;
		case Defpoint: src = cp->u.point.src; break;
		case Gen: case Jump:
		case Label:    if (!g_code_generator->wants_dag)
			       	cp->u.forest = undag(cp->u.forest);
			       fixup(cp->u.forest);
			       cp->u.forest = (*g_code_generator->gen)(cp->u.forest); break;
		case Local:    (*g_code_generator->local)(cp->u.var); break;
		case Switch:   break;
		default: assert(0);
		}
	src = save;
...
void emitcode() {
	...
	for ( ; errcnt <= 0 && cp; cp = cp->next)
		switch (cp->kind) {
		case Address: break;
		case Blockbeg: if (glevel && g_code_generator->stabblock) {
			       	(*g_code_generator->stabblock)('{',  cp->u.block.level - C_LocalScope, cp->u.block.locals);
			       	swtoseg(CODE);
			       }
 break;
		case Blockend: if (glevel && g_code_generator->stabblock) {
			       	Code bp = cp->u.begin;
			       	foreach(bp->u.block.identifiers, bp->u.block.level, typestab, NULL);
			       	foreach(bp->u.block.types,       bp->u.block.level, typestab, NULL);
			       	(*g_code_generator->stabblock)('}', bp->u.block.level - C_LocalScope, bp->u.block.locals);
			       	swtoseg(CODE);
			       }
 break;
		case Defpoint: src = cp->u.point.src;
			       if (glevel > 0 && g_code_generator->stabline) {
			       	(*g_code_generator->stabline)(&cp->u.point.src); swtoseg(CODE); } break;
		case Gen: case Jump:
		case Label:    if (cp->u.forest)
			       	(*g_code_generator->emit)(cp->u.forest); break;
		case Local:    if (glevel && g_code_generator->stabsym) {
			       	(*g_code_generator->stabsym)(cp->u.var);
			       	swtoseg(CODE);
			       } break;
		case Switch:   {	int i;
			       	defglobal(cp->u.swtch.table, LIT);
			       	(*g_code_generator->defaddress)(equated(cp->u.swtch.labels[0]));
			       	for (i = 1; i < cp->u.swtch.size; i++) {
			       		long k = cp->u.swtch.values[i-1];
			       		while (++k < cp->u.swtch.values[i])
			       			assert(k < LONG_MAX),
			       			(*g_code_generator->defaddress)(equated(cp->u.swtch.deflab));
			       		(*g_code_generator->defaddress)(equated(cp->u.swtch.labels[i]));
			       	}
			       	swtoseg(CODE);
			       } break;
		default: assert(0);
...
static Node visit(p, listed) Node p; int listed; {
	if (p)
		...
		&&  p->kids[0]->syms[0]->sclass == REGISTER) {
			...
			p->kids[0] = visit(p->kids[0], 0);
			p->kids[1] = visit(p->kids[1], 0);
			p->syms[2] = temporary(REGISTER, btot(p->op), C_LocalScope);
			assert(!p->syms[2]->defined);
			p->syms[2]->ref = 1;
			p->syms[2]->u.t.cse = p;
			(*g_code_generator->local)(p->syms[2]);
			p->syms[2]->defined = 1;
...
static void printdag1(p, fd, lev) Node p; int fd, lev; {
	...
	if (p == 0 || *printed(id = nodeid((Tree)p)))
		return;
	*printed(id) = 1;
	for (i = 0; i < D_ArrayLength(p->kids); i++)
...
static void printnode(p, fd, lev) Node p; int fd, lev; {
	if (p) {
		...
		for (i = 0; i < D_ArrayLength(p->kids) && p->kids[i]; i++)
			fprint(fd, " #%d", nodeid((Tree)p->kids[i]));
		for (i = 0; i < D_ArrayLength(p->syms) && p->syms[i]; i++)
...
static void typestab(p, cl) Symbol p; void *cl; {
	if (!isfunc(p->type) && (p->sclass == EXTERN || p->sclass == STATIC) && g_code_generator->stabsym)
		(*g_code_generator->stabsym)(p);
	else if ((p->sclass == TYPEDEF || p->sclass == 0) && g_code_generator->stabtype)
		(*g_code_generator->stabtype)(p);
}
