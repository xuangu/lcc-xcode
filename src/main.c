...
// 该字段没有什么具体的含义
static char version_ctrl_info[] = "lcc $Name: v3_6 $($Id$)";
...
// lcc支持交叉编译，支持为不同的平台生成目标代码
// 目标平台的代码生成器（入口函数根据lcc命令的编译选项进行赋值），lcc根据该生成器为不同的平台生成目标代码
CodeGenerationInterface *g_code_generator = NULL;
...
int main(argc, argv) int argc; char *argv[]; {
    // 根据编译选项生成目标代码生成器
    {
		...
		for (i = argc - 1; i > 0; i--)
			if (strncmp(argv[i], "-target=", 8) == 0)
				break;
		if (i > 0) {
			for (j = 0; supported_code_generation_options[j].name; j++)
				if (strcmp(&argv[i][8], supported_code_generation_options[j].name) == 0)
					break;
			if (supported_code_generation_options[j].ir)
				g_code_generator = supported_code_generation_options[j].ir;
			else {
	...
	if (!g_code_generator) {
		...
		for (i = 0; supported_code_generation_options[i].name; i++)
			fprint(2, "\t-target=%s\n", supported_code_generation_options[i].name);
		exit(1);
	}
	...
	outputInit();
	t = gettok();
	(*g_code_generator->progbeg)(argc, argv);
	if (glevel && g_code_generator->stabinit)
		(*g_code_generator->stabinit)(firstfile, argc, argv);
	program();
	if (events.end)
	...
	if (glevel || xref) {
		Symbol symroot = NULL;
		Coordinate src;
		foreach(types,       C_GlobalScope, typestab, &symroot);
		foreach(identifiers, C_GlobalScope, typestab, &symroot);
		src.file = firstfile;
		src.x = 0;
		src.y = lineno;
		if ((glevel > 2 || xref) && g_code_generator->stabend)
			(*g_code_generator->stabend)(&src, symroot,
				ltov(&loci,    PERM),
				ltov(&symbols, PERM), NULL);
		else if (g_code_generator->stabend)
			(*g_code_generator->stabend)(&src, NULL, NULL, NULL, NULL);
	}
	finalize();
	(*g_code_generator->progend)();
	outflush();
	close(infd);
	close(outfd);
	close(errfd);
	...
	for (i = j = 1; i < argc; i++)
		...
		&& argv[i][2] && argv[i][2] >= '0' && argv[i][2] <= '9') {
...
#ifdef STABINIT
			{
				...
				g_code_generator->stabinit = STABINIT;
			}
#endif
		...
		||       strncmp(argv[i], "-a", 2) == 0)
			...
			if (!YYnull) {
				YYnull = add_symbol(string("_YYnull"), &globals, C_GlobalScope, PERM);
				YYnull->type = ftype(voidtype, inttype);
				YYnull->sclass = STATIC;
				(*g_code_generator->defsymbol)(YYnull);
			}
			...
			int i;
			fprint(2, "%s %s targets:\n", argv[0], version_ctrl_info);
			for (i = 0; supported_code_generation_options[i].name; i++)
				fprint(2, "\t%s%s\n", supported_code_generation_options[i].name,
					g_code_generator == supported_code_generation_options[i].ir ? "*" : "");
			...
			g_code_generator->wants_dag = !g_code_generator->wants_dag;
...
static void typestab(p, cl) Symbol p; void *cl; {
	if (*(Symbol *)cl == 0 && p->sclass && p->sclass != TYPEDEF)
		*(Symbol *)cl = p;
	if ((p->sclass == TYPEDEF || p->sclass == 0) && g_code_generator->stabtype)
