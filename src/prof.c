...
static void bbcall(yycounts, cp, e) Symbol yycounts; Coordinate *cp; Tree *e; {
	...
	Value v;
	union coordinate u;
	Symbol p = genident(STATIC, array(voidptype, 0, 0), C_GlobalScope);
	...
	(*g_code_generator->defconst)(U, (v.u = u.coord, v));
	if (caller == 0) {
...
static void bbentry(yylink, f) Symbol yylink, f; {
	...
	afunc = genident(STATIC, array(voidptype, 4, 0), C_GlobalScope);
	if (p == 0) {
...
static void bbvars(yylink) Symbol yylink; {
	...
	files = genident(STATIC, array(ptr(chartype), 1, 0), C_GlobalScope);
	defglobal(files, LIT);
	for (p = ltov(&filelist, PERM); *p; p++)
		defpointer((*p)->u.c.loc);
	defpointer(NULL);
	coords = genident(STATIC, array(unsignedtype, n, 0), C_GlobalScope);
	defglobal(coords, LIT);
	for (i = n, mp = ltov(&maplist, PERM); *mp; i -= (*mp)->size, mp++)
		for (j = 0; j < (*mp)->size; j++)
			(*g_code_generator->defconst)(U, (v.u = (*mp)->u[j].coord, v));
	if (i > 0)
		(*g_code_generator->space)(i*coords->type->type->size);
	defpointer(NULL);
	defglobal(yylink, DATA);
	defpointer(NULL);
	(*g_code_generator->defconst)(U, (v.u = n, v));
	defpointer(YYcounts);
	defpointer(coords);
	defpointer(files);
	defpointer(funclist);
...
void profInit(opt) char *opt; {
	if (strncmp(opt, "-a", 2) == 0) {
		...
		YYlink = genident(STATIC, array(unsignedtype, 0, 0), C_GlobalScope);
		...
		if (strcmp(opt, "-b") == 0) {
			YYcounts = genident(STATIC, array(unsignedtype, 0, 0), C_GlobalScope);
