...
#define HASHSIZE D_ArrayLength(((Table)0)->buckets)
static struct table
	cns = { C_ConstantScope },
	ext = { C_GlobalScope },
	ids = { C_GlobalScope },
...
void exitscope() {
	...
	assert(level >= C_GlobalScope);
	--level;
}


/**
 * @description: 在符号表中新增符号
 * @param {char *} name 新增符号标识符
 * @param {Table *} table_entry 存储符号符号表入口
 * @param {int} scope_level 符号的作用域level
 * @param {int} storage_arena 存储区域
 * @return {Symbol} 新增的符号
 */
Symbol add_symbol(char *name, Table *table_entry, int scope_level, int storage_arena) {
	Table table = *table_entry;
	struct entry *entry;
	unsigned hash = (unsigned long)name&(HASHSIZE-1);

	assert(scope_level == 0 || scope_level >= table->level);
	if (level > 0 && tp->level < scope_level)
...
Symbol findlabel(lab) int lab; {
	...
	p->sym.generated = 1;
	p->sym.u.l.label = lab;
	(*g_code_generator->defsymbol)(&p->sym);
	return &p->sym;
}
...
Symbol genident(scls, ty, lev) int scls, lev; Type ty; {
	...
	NEW0(p, lev >= C_LocalScope ? FUNC : PERM);
	p->name = stringd(genlabel(1));
	p->scope = lev;
	p->sclass = scls;
	p->type = ty;
	p->generated = 1;
	if (lev == C_GlobalScope)
...
Symbol newtemp(sclass, tc) int sclass, tc; {
	Symbol p = temporary(sclass, btot(tc), C_LocalScope);
...
Symbol mkstr(str) char *str; {
	...
	if (p->u.c.loc == NULL)
		p->u.c.loc = genident(STATIC, p->type, C_GlobalScope);
	return p;
}

/* mksymbol - make a symbol for name, add_symbol in &globals if sclass==EXTERN */
Symbol mksymbol(sclass, name, ty) int sclass; char *name; Type ty; {
	Symbol p;

	if (sclass == EXTERN)
		p = add_symbol(string(name), &globals, C_GlobalScope, PERM);
	else {
		NEW0(p, PERM);
		p->name = string(name);
