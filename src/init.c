...
void defpointer(p) Symbol p; {
	if (p)
		(*g_code_generator->defaddress)(p);
	else {
		Value v;
		(*g_code_generator->defconst)(P, (v.p = 0, v));
	}
}
...
static void genchar(p, sp) Symbol p; struct structexp *sp; {
	...
	if (n > 0) {
		...
		p = constant(ty, v);
		if (p->u.c.loc == NULL)
			p->u.c.loc = genident(STATIC, ty, C_GlobalScope);
...
static int genconst(e, def) Tree e; int def; {
	for (;;)
		...
		case ADDRG:
			if (def)
				(*g_code_generator->defaddress)(e->u.sym);
			return e->type->size;
		case CNST:
			...
			if (def)
				(*g_code_generator->defconst)(ttob(e->type), e->u.v);
			return e->type->size;
		case RIGHT:
...
static void genspace(n, sp) int n; struct structexp *sp; {
	if (sp == 0)
		...
		static Symbol zeros;
		if (zeros == 0) {
			zeros = add_symbol(stringd(genlabel(1)), &globals, C_GlobalScope, PERM);
			zeros->sclass = STATIC;
			zeros->type = array(chartype, n, 0);
			zeros->generated = 1;
			(*g_code_generator->defsymbol)(zeros);
		}
...
static int initchar(len, ty) Type ty; int len; {
	...
	do {
		if (current) {
			...
			*s++ = initvalue(ty)->u.v.sc;
			if (++n%inttype->size == 0) {
				(*g_code_generator->defstring)(inttype->size, buf);
				s = buf;
			}
		}
		if (len > 0 && n >= len || t != ',')
	...
	if (s > buf)
		(*g_code_generator->defstring)(s - buf, buf);
	return n;
}
	...
	if (isarray(ty) && ischar(aty)) {
		if (t == SCON) {
			...
			if (current)
				...
				(*g_code_generator->defstring)(tsym->type->size, tsym->u.c.v.p);
			t = gettok();
		} else if (t == '{') {
			t = gettok();
			if (t == SCON) {
