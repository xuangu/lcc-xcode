...
CodeGenerationInterface symbolicIR = {
	1, 1, 0,	/* char */
	2, 2, 0,	/* short */
	4, 4, 0,	/* int */
	4, 4, 1,	/* float */
...
static void defsymbol(p) Symbol p; {
	if (p->scope == C_ConstantScope)
		...
		p->x.name = p->name;
	if (glevel > 2 && p->scope >= C_LocalScope && p->type && isfunc(p->type))
		sym("extern", p, "\n");
}
...
static Node gen0(p) Node p; {
	...
	for (n = 0; p; p = p->link) {
		...
		case CALL:
			assert(g_code_generator->wants_dag || p->count == 0);
			break;
		case ARG:
...
static void progbeg(argc, argv) int argc; char *argv[]; {
	...
	for (i = 0; i < argc; i++)
		if (strncmp(argv[i], "-little_endian=", 15) == 0)
			g_code_generator->little_endian = argv[i][15] - '0';
		else if (strncmp(argv[i], "-mulops_calls=", 18) == 0)
			g_code_generator->mulops_calls = argv[i][18] - '0';
		else if (strncmp(argv[i], "-wants_callb=", 13) == 0)
			g_code_generator->wants_callb = argv[i][13] - '0';
		else if (strncmp(argv[i], "-wants_argb=", 12) == 0)
			g_code_generator->wants_argb = argv[i][12] - '0';
		else if (strncmp(argv[i], "-left_to_right=", 15) == 0)
			g_code_generator->left_to_right = argv[i][15] - '0';
		else if (strncmp(argv[i], "-wants_dag=", 11) == 0)
			g_code_generator->wants_dag = argv[i][11] - '0';
}
...
static void sym(kind, p, str) char *kind, *str; Symbol p; {
	...
	switch (p->scope) {
	case C_ConstantScope: print("C_ConstantScope"); break;
	case C_LabelScope:    print("C_LabelScope");    break;
	case C_GlobalScope:    print("C_GlobalScope");    break;
	case C_ParameterScope:     print("C_ParameterScope");     break;
	case C_LocalScope:     print("C_LocalScope");     break;
	default:
		if (p->scope > C_LocalScope)
			print("C_LocalScope+%d", p->scope - C_LocalScope);
		else
			print("%d", p->scope);
	}
	if (p->scope >= C_ParameterScope && p->sclass != STATIC)
