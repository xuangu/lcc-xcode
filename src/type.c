...
Type int_type;			/* signed int */
Type long_double_type;		/* long double */
Type long_type;			/* long */
Type short_type;			/* signed short int */
Type signed_char_type;		/* signed char */
Type unsigned_char_type;		/* unsigned char */
Type unsigned_long_type;		/* unsigned long int */
Type unsigned_short_type;		/* unsigned short int */
Type unsigned_type;		/* unsigned int */
Type void_pointer_type;			/* void* */
Type void_type;			/* basic types: void */

static Type new_type(int type_tag, Type type, int size, int align, void *symbol) {
	unsigned hash_value = (type_tag ^ ((unsigned long)type >> 3)) & (D_ArrayLength(type_table) - 1);
	struct entry *entry;
	if (type_tag != C_Function && (type_tag != C_Array || size > 0)) {
		for (entry = type_table[hash_value]; entry; entry = entry->link) {
			if (entry->type.tag == type_tag 
            && entry->type.base_type  == type
            && entry->type.object_size == size 
            && entry->type.align_size == align
            && entry->type.ext_info.symbol == symbol) {
				return &entry->type;
            }
        }
    }
	D_NewAndClear(entry, C_MemoryPermanentZone);
	entry->type.tag = type_tag;
	entry->type.base_type = type;
	entry->type.object_size = size;
	entry->type.align_size = align;
	entry->type.ext_info.symbol = symbol;
	entry->link = type_table[hash_value];
	type_table[hash_value] = entry;
	return &entry->type;
}

inline static void install_base_type(Type base_type, char *name, int tag, Metrics metrics) {
    Symbol symbol = add_symbol(string(name), &types, C_GlobalScope, C_MemoryPermanentZone);
    Type type = new_type(tag, base_type, metrics.size, metrics.align, symbol);
    assert(type->align_size == 0 || type->object_size % type->align_size == 0);
    symbol->type = type;
    symbol->addressed = metrics.outofline;
}
...
void type_init() {
	...
	struct type_info type_infos[] = {
		{chartype, "char", CHAR, charmetric};
	};

#define xx(v,name,op,metrics) { \
		Symbol p = add_symbol(string(name), &types, C_GlobalScope, C_MemoryPermanentZone);\
		v = new_type(op, 0, g_code_generator->metrics.size, g_code_generator->metrics.align, p);\
		assert(v->align == 0 || v->size%v->align == 0); \
		p->type = v; p->addressed = g_code_generator->metrics.outofline; }
...
#undef xx
	{
		Symbol p;
		p = add_symbol(string("void"), &types, C_GlobalScope, PERM);
		voidtype = type(VOID, NULL, 0, 0, p);
		p->type = voidtype;
	}
	pointersym = add_symbol(string("T*"), &types, C_GlobalScope, PERM);
	pointersym->addressed = g_code_generator->ptrmetric.outofline;
...
void rmtypes(lev) int lev; {
	if (maxlevel >= lev) {
		int i;
		maxlevel = 0;
		for (i = 0; i < D_ArrayLength(type_table); i++) {
			struct entry *tn, **tq = &type_table[i];
			while ((tn = *tq) != NULL)
...
Type ptr(ty) Type ty; {
	return type(POINTER, ty, g_code_generator->ptrmetric.size,
		g_code_generator->ptrmetric.align, pointersym);
}
...
Type array(ty, n, a) Type ty; int n, a; {
	...
	if (level > C_GlobalScope && isarray(ty) && ty->size == 0)
...
Type newstruct(op, tag) int op; char *tag; {
	...
	if (*tag == 0)
		...
		|| p->scope == C_ParameterScope && level == C_ParameterScope+1)) {
			if (p->type->op == op && !p->defined)
				return p->type;
			error("redefinition of `%s' previously defined at %w\n",
				p->name, &p->src);
		}
	p = add_symbol(tag, &types, level, PERM);
...
Field newfield(name, ty, fty) char *name; Type ty, fty; {
	...
	*q = p;
	...
	if (xref) {							/* omit */
		...
		add_symbol(name, &ty->u.sym->u.s.ftab, 0, PERM)->src = src;/* omit */
	}								/* omit */
	return p;
}
