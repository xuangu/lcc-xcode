 ...
 break;
	case SCON: tsym->u.c.v.p = stringn(tsym->u.c.v.p, tsym->type->size);
		   ...
		   if (tsym->u.c.loc == NULL)
		   	tsym->u.c.loc = genident(STATIC, tsym->type, C_GlobalScope);
		   p = idtree(tsym->u.c.loc); break;
	case ID:   if (tsym == NULL)
		   	{
				Symbol q = add_symbol(token, &identifiers, level, level < C_LocalScope ? PERM : FUNC);
				q->src = src;
				t = gettok();
				if (t == '(') {
					Symbol r;
					...
					(*g_code_generator->defsymbol)(q);
				 	if ((r = lookup(q->name, externals)) != NULL) {
						...
						r = add_symbol(q->name, &externals, C_GlobalScope, PERM);
						r->src = q->src;
						r->type = q->type;
						r->sclass = EXTERN;
					}
...
Tree idtree(p) Symbol p; {
	...
	||  p->sclass == STATIC || p->sclass == EXTERN)
		...
		op = ADDRF+P;
		if (isstruct(p->type) && !g_code_generator->wants_argb)
			{
...
int hascall(p) Tree p; {
	if (p == 0)
		return 0;
	if (generic(p->op) == CALL || (g_code_generator->mulops_calls &&
