#define D_New(p,a) ((p) = allocate(sizeof *(p), (a)))
#define D_NewAndClear(p,a) memset(NEW((p),(a)), 0, sizeof *(p))
...
#endif
#endif
#define D_ArrayLength(a) ((int)(sizeof (a)/sizeof ((a)[0])))
...
//
typedef struct code_generation_option {
	char *name;
	CodeGenerationInterface *ir;
} CodeGenerationOption;

extern CodeGenerationOption supported_code_generation_options[];
extern CodeGenerationInterface *g_code_generator;
typedef struct {
	List entry;
	List exit;
	List returns;
...
typedef enum { 
	C_MemoryPermanentZone=0, 
	C_MemoryFunctionZone, 
	C_MemoryStatementZone 
} StorageZone;
...
// 类型采用如下形式表示：(C_Pointer (C_Array 10 (C_Pointer (C_Char))))
struct type {
	int tag;     // 类型描述符编码，e.g. C_Char
	Type base_type;      // 嵌套的类型（如果有）
	int align_size;      // 该类型对齐字节数，单位字节
	int object_size;       // 该类型对象的字节数，单位字节
    // 一些扩展信息
    union {
        // 该字段给出更多与类型有关的信息
        Symbol symbol;
        ...
        struct {
            // 是否是K&R风格的函数声明
            unsigned k_and_r_style:1;
            Type *prototype;
        } func_params_info;
    } ext_info;
    // 目标机器信息，编译器后端使用此字段
	SpecificTargetMachineType target_use_type_info;
};
...
extern Symbol add_symbol ARGS((char *, Table *, int, int));
