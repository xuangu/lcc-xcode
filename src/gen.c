...
int dreg, doff, sreg, soff, size, tmp[]; {
	...
	if (size == 0)
		...
		(*g_code_generator->x.blkloop)(dreg, doff, sreg, soff, size, tmp);
}
static void blkunroll(k, dreg, doff, sreg, soff, size, tmp)
int k, dreg, doff, sreg, soff, size, tmp[]; {
	int i;

	assert(g_code_generator->x.max_unaligned_load);
	if (k > g_code_generator->x.max_unaligned_load
	&& (k > salign || k > dalign))
		k = g_code_generator->x.max_unaligned_load;
	for (i = 0; i+k < size; i += 2*k) {
		(*g_code_generator->x.blkfetch)(k, soff+i,   sreg, tmp[0]);
		(*g_code_generator->x.blkfetch)(k, soff+i+k, sreg, tmp[1]);
		(*g_code_generator->x.blkstore)(k, doff+i,   dreg, tmp[0]);
		(*g_code_generator->x.blkstore)(k, doff+i+k, dreg, tmp[1]);
	}
	if (i < size) {
		(*g_code_generator->x.blkfetch)(k, i+soff, sreg, tmp[0]);
		(*g_code_generator->x.blkstore)(k, i+doff, dreg, tmp[0]);
	}
}
...
static int getrule(p, nt) Node p; int nt; {
	...
	assert(p);
	rulenum = (*g_code_generator->x._rule)(p->x.state, nt);
	assert(rulenum);
	return rulenum;
}
static void reduce(p, nt) Node p; int nt; {
	...
	nts = g_code_generator->x._nts[rulenum];
	(*g_code_generator->x._kids)(p, rulenum, kids);
	for (i = 0; nts[i]; i++)
		reduce(kids[i], nts[i]);
	if (g_code_generator->x._isinstruction[rulenum]) {
...
static void dumptree(p) Node p; {
	fprint(2, "%s(", g_code_generator->x._opname[p->op]);
	if (g_code_generator->x._arity[p->op] == 0 && p->syms[0])
		fprint(2, "%s", p->syms[0]->name);
	else if (g_code_generator->x._arity[p->op] == 1)
		dumptree(p->kids[0]);
	else if (g_code_generator->x._arity[p->op] == 2) {
		dumptree(p->kids[0]);
		fprint(2, ", ");
		dumptree(p->kids[1]);
	}
...
static void dumprule(rulenum) int rulenum; {
	assert(rulenum);
	fprint(2, "%s / %s", g_code_generator->x._string[rulenum],
		g_code_generator->x._templates[rulenum]);
	if (!g_code_generator->x._isinstruction[rulenum])
		fprint(2, "\n");
}
static unsigned emitasm(p, nt) Node p; int nt; {
	...
	rulenum = getrule(p, nt);
	nts = g_code_generator->x._nts[rulenum];
	fmt = g_code_generator->x._templates[rulenum];
	assert(fmt);
	if (g_code_generator->x._isinstruction[rulenum] && p->x.emitted)
		outs(p->syms[RX]->x.name);
	else if (*fmt == '#')
		(*g_code_generator->x.emit2)(p);
	else {
		if (*fmt == '?') {
			fmt++;
			assert(p->x.kids[0]);
		...
		for ((*g_code_generator->x._kids)(p, rulenum, kids); *fmt; fmt++)
			if (*fmt != '%')
				*bp++ = *fmt;
			else if (*++fmt == 'F')
				print("%d", framesize);
			...
			else if (*fmt >= 'a' && *fmt < 'a' + D_ArrayLength(p->syms))
...
static void ralloc(p) Node p; {
	...
	&& rmap[optype(p->op)]) {
		...
		if (set->sclass != REGISTER) {
			Symbol r;
			if (*g_code_generator->x._templates[getrule(p, p->x.inst)] == '?')
				for (i = 1; i < D_ArrayLength(p->x.kids) && p->x.kids[i]; i++) {
...
static int uses(p, mask) Node p; unsigned mask; {
	...
	for (i = 0; i < D_ArrayLength(p->x.kids)
		&& (q = p->x.kids[i]) != NULL; i++)
		if (q->x.registered
